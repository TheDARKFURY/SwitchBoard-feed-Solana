...
fn main() {
    ...
    match option {
        1u8 => {
            let feed_vec_pubkey = instructions::create_feed_vec().expect("err");
            feed_vector_pubkey.push(feed_vec_pubkey);
        }
        2u8 => instructions::read_sol_feed(feed_vector_pubkey.last().unwrap()),
        3u8 => instructions::cal_data_spread(feed_vector_pubkey.last().unwrap()),
        4u8 => instructions::reset_feed_vec_acc(feed_vector_pubkey.last().unwrap()),
        _ => {
...
const FEED_VEC: &str = "DVhtZaQg5VnFAhm9aR9hucsj1mkM8oW8TH2XKzvT3ZUc";

mod instructions {
    use super::*;
    pub fn create_feed_vec() -> Result<pubkey::Pubkey, String> {
        ...
        Ok(feed_vector_pubkey)
    }
    pub fn read_sol_feed(feed_vec_pubkey: &pubkey::Pubkey) {
        ...
        let sig1 = program
            ...
            .accounts(ReadResult_Acc::ReadHistorybuffer {
                ...
                feed_vec_acc: feed_vec_pubkey.to_owned(),
                authority: payer_pubkey,
            })
    ...
    pub fn cal_data_spread(feed_vec_pubkey: &pubkey::Pubkey) {
        let feed_pid = pubkey::Pubkey::from_str(SOLANA_FEED_PID).unwrap();
        ...
        let sig1 = program
            ...
            .accounts(ReadResult_Acc::CalculateDataSpread {
                feed_vector_acc: feed_vec_pubkey.to_owned(),
                authority: payer_pubkey,
            })
            .args(ReadResult_Ins::CalculateDataSpread {})
            .send()
            .unwrap();
    ...
    pub fn reset_feed_vec_acc(feed_vec_pubkey: &pubkey::Pubkey) {
        let feed_pid = pubkey::Pubkey::from_str(SOLANA_FEED_PID).unwrap();
